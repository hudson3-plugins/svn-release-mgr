...
public class SubversionReleaseSCM extends SCM implements Serializable {
    ...
    public SubversionReleaseSCM(String[] remoteLocations, String[] localLocations,
    ...
    public boolean checkout(AbstractBuild<?, ?> build, Launcher launcher, FilePath workspace, final BuildListener listener, File changelogFile) throws IOException, InterruptedException {
        ...
        try {
            ...
            build.addAction(new SubversionReleaseTagAction(build,revMap.values()));
        } finally {
            w.close();
        }
                ...
                if(update) {
                    for (final ModuleLocation l : locations) {
                        try {
                            ...
                            svnuc.setEventHandler(new SubversionReleaseUpdateEventHandler(listener.getLogger(), externals,local,l.local));
                    ...
                    for (final ModuleLocation l : locations) {
                        try {
                            ...
                            File local = new File(ws, l.local);
                            svnuc.setEventHandler(new SubversionReleaseUpdateEventHandler(new PrintStream(pos), externals, local, l.local));
    ...
    public static class DescriptorImpl extends SCMDescriptor<SubversionReleaseSCM> {
        ...
        public SCM newInstance(StaplerRequest req, JSONObject formData) throws FormException {
            return new SubversionReleaseSCM(
                req.getParameterValues("svn-r.location_remote"),
                req.getParameterValues("svn-r.location_local"),false,
            ...
            new FormFieldValidator(req, rsp, false) {
                protected void check() throws IOException, ServletException {
                    ...
                    if (!Hudson.getInstance().hasPermission(Hudson.ADMINISTER)) {
                        ...
                        message += " (Maybe you need to <a target='_new' href='"+req.getContextPath()+"/scm/SubversionReleaseSCM/enterCredential?"+url+"'>enter credential</a>?)";
                        message += "<br>";
